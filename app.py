...
app = FastAPI(
    ...
    description="Generates and auto-activates a Free Fire guest account based on OB52 Fixed Logic.",
    version="2.1.0"
)
...
def extract_jwt_auth_data(jwt_token: str):
    """Extracts external_id and signature_md5 needed for account activation."""
    try:
        parts = jwt_token.split('.')
        if len(parts) >= 2:
            payload_part = parts[1]
            payload_part += "=" * ((4 - len(payload_part) % 4) % 4)
            decoded_jwt = json.loads(base64.urlsafe_b64decode(payload_part).decode('utf-8'))
            eid = str(decoded_jwt.get('external_id', ''))
            sig = str(decoded_jwt.get('signature_md5', ''))
            return eid, sig
    except Exception as e:
        logging.error(f"JWT Extract Error: {e}")
    return "", ""
...
def generate_account(
    ...
    try:
        ...
        # --- 6. AUTOMATIC ACCOUNT ACTIVATION ---
        is_active = False
        eid, sig = extract_jwt_auth_data(jwt_token)
        
        if eid and sig:
            try:
                base_hex = ""
                pl_bytes = bytes.fromhex(base_hex)
                
                now_bytes = datetime.now().strftime('%Y-%m-%d %H:%M:%S').encode()
                pl_bytes = pl_bytes.replace(b"2025-07-30 11:02:51", now_bytes)
                pl_bytes = pl_bytes.replace(b"ff90c07eb9815af30a43b4a9f6019516e0e4c703b44092516d0defa4cef51f2a", access_token.encode())
                pl_bytes = pl_bytes.replace(b"996a629dbcdb3964be6b6978f5d814db", eid.encode())
                pl_bytes = pl_bytes.replace(b"7428b253defc164018c604a1ebbfebdf", sig.encode())
                
                enc_payload_active = bytes.fromhex(encrypt_api(pl_bytes.hex()))
                
                # Setup specific endpoints based on region
                client_url = "https://client.ind.freefiremobile.com/GetLoginData" if region_code == "IND" else "https://clientbp.ggblueshark.com/GetLoginData"
                ...
                res_active = req_session.post(client_url, headers=headers_active, data=enc_payload_active, verify=False, timeout=10)
                if res_active.status_code == 200:
                    is_active = True
                else:
                    logging.warning(f"Activation Failed: Client Rejection HTTP {res_active.status_code}")
            except Exception as e:
                logging.error(f"Activation Request Error: {e}")

        # Return Final Success JSON
        return AccountResponse(
            status="success",
            message="Account generated and activation logic applied.",
            is_active=is_active,
        ...
        logging.error(f"Generation/Activation Error: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Account Process Failed: {str(e)}")
