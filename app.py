...
app = FastAPI(
    ...
    description="Generates a Free Fire guest account and perfectly activates it using OB52 Fixed Logic.",
    version="3.0.0"
)
...
# --- AUTO ACTIVATION LOGIC (FROM active.py) ---
def gen_activation_payload(jwt: str, guest_token: str) -> bytes:
    try:
        parts = jwt.split('.')
        decoded = json.loads(base64.urlsafe_b64decode(parts[1] + '==').decode('utf-8'))
        eid = str(decoded.get('external_id', ''))
        sig = str(decoded.get('signature_md5', ''))
        ...
        pl = pl.replace(b"ff90c07eb9815af30a43b4a9f6019516e0e4c703b44092516d0defa4cef51f2a", guest_token.encode())
...
def execute_account_activation(uid: str, password: str, region_code: str):
    """Executes the exact activation flow mirroring active.py perfectly."""
    ...
    cfg = regions_cfg.get(region_code, regions_cfg['BD'])
    
    session = requests.Session()
    session.verify = False
    
    # Step 1: Fresh Guest Token
    try:
        data = {
        ...
        resp = session.post(cfg['g'], data=data, timeout=10)
        if resp.status_code != 200: return False, None
        js = resp.json()
        tok = js.get('access_token')
        oid = js.get('open_id')
        if not tok or not oid: return False, None
    except Exception as e: 
        logging.error(f"Activation Phase (Guest Token) Failed: {e}")
        return False, None

    # Step 2: Fresh Major Login (Using strict active.py hex payload)
    headers_ml = {
        ...
        'Host': 'loginbp.ggblueshark.com' if region_code not in ['IND', 'ME'] else cfg['m'].split('/')[2], 
    ...
    try:
        ...
        resp = session.post(cfg['m'], headers=headers_ml, data=bytes.fromhex(enc), timeout=10)
        if resp.status_code != 200: return False, None
        major_data = resp.content
    except Exception as e: 
        logging.error(f"Activation Phase (Major Login) Failed: {e}")
        return False, None

    # Step 3: Decrypt Major Login & Extract Valid JWT
    jwt = ""
    decrypted_data = decrypt_api(major_data)
    data_to_parse = decrypted_data if decrypted_data else major_data
    ...
    if not jwt:
        try:
            text_data = data_to_parse.decode('latin1', errors='ignore')
            m = re.search(r'eyJ[\w\-]+\.eyJ[\w\-]+\.[\w\-]+', text_data)
            if m: jwt = m.group(0)
        except: pass
        
    if not jwt: return False, None

    # Step 4: Final Client Activation Payload
    payload_client = gen_activation_payload(jwt, tok)
    if not payload_client: return False, jwt
    ...
    try:
        r = session.post(cfg['c'], headers=head_client, data=payload_client, timeout=10)
        if r.status_code == 200:
            return True, jwt
    except Exception as e:
        logging.error(f"Activation Phase (Client Data) Failed: {e}")
...
def generate_account(
    ...
    try:
        # --- 1. GUEST REGISTER (GENERATION PHASE) ---
        ...
        if 'uid' not in res_reg_json: raise ValueError("Guest Register Failed: No UID returned")
        ...
        req_session.post(f"https://{url_host}/MajorRegister", headers=headers_major, data=enc_payload_mr, verify=False, timeout=15)
        
        # --- 4. MAJOR LOGIN (INITIAL REGISTRATION LOGIC) ---
        ml_payload_parts = [
        ...
        if res_ml.status_code == 200:
            if PROTOBUF_AVAILABLE:
                try:
                    res = MajorLoginRes()
                    res.ParseFromString(res_ml.content)
                    gen_jwt = res.token
                except: pass
            if not gen_jwt:
                m = re.search(r'eyJ[\w\-]+\.eyJ[\w\-]+\.[\w\-]+', res_ml.text)
                if m: gen_jwt = m.group(0)
            account_id = str(decode_jwt_token(gen_jwt))
        ...
        if region_code != "BR" and gen_jwt:
            ...
            headers_choose["Authorization"] = f"Bearer {gen_jwt}"
        ...
        # =========================================================
        # --- 6. AUTO ACTIVATION PHASE (USING active.py LOGIC) ---
        # =========================================================
        is_active, final_jwt = execute_account_activation(uid, password, region_code)
        
        # Fallback to generation JWT if activation returned none
        returned_jwt = final_jwt if final_jwt else gen_jwt

        return AccountResponse(
            status="success",
            message="Account generated and perfectly activated." if is_active else "Account generated but auto-activation failed.",
            is_active=is_active,
