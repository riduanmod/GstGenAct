...
# Import variables from game_version.py
...
app = FastAPI(
    ...
    description=f"Generates a Free Fire guest account based on {RELEASE_VERSION} Fixed Logic and seamlessly activates it.",
    version="2.1.0"
)
...
def activate_account(session: requests.Session, uid: str, pwd: str, region: str = "BD"):
    ...
    headers = {
        ...
        'X-GA': 'v1 1', 
        'User-Agent': f'Dalvik/2.1.0 (Linux; U; {ANDROID_OS_VERSION}; {USER_AGENT_MODEL})', 
...
def generate_and_activate_account(
    ...
    try:
        ...
        # --- 4. MAJOR LOGIN ---
        # Note: The hex data here still contains static payload representations. We only changed the request headers 
        # which are the key validation parameters on the server side.
        ml_payload_parts = [
