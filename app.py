...
app = FastAPI(
    ...
    description="Generates and immediately activates a Free Fire guest account with exact name based on OB52 Fixed Logic.",
    version="2.2.0"
)
...
def generate_activation_payload(jwt_token: str, access_token: str) -> bytes:
    try:
        ...
        if not eid or not sig:
            logging.error("Missing external_id or signature_md5 in JWT for activation payload.")
            return None
...
def generate_account(
    ...
    # ১০০% হুবহু ইউজারের দেওয়া নাম ব্যবহার করা হয়েছে (কোনো রান্ডম সংখ্যা যুক্ত হবে না)
    ...
    try:
        ...
        # টার্মিনাল/লগিং এ এক্সেস টোকেন এবং ওপেন আইডি প্রিন্ট করা হচ্ছে
        logging.info("=" * 45)
        ...
        if res_ml.status_code == 200 and len(res_ml.content) > 0:
            ...
            # নিখুঁতভাবে রেগুলার এক্সপ্রেশন (Regex) দিয়ে JWT বের করার সিস্টেম (অ্যাক্টিভেশন ফেইল রিমুভ করার জন্য)
        ...
        # --- 6. ACTIVATE ACCOUNT ---
        is_activated = False
        activation_msg = "Account created but activation logic failed."
        ...
        if act_payload:
            ...
            try:
                ...
                if res_act.status_code == 200:
                    is_activated = True
                    activation_msg = "Account generated and successfully activated."
                else:
                    logging.error(f"Activation Failed: Status Code {res_act.status_code}")
            except Exception as e:
